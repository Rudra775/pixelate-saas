...
import SmartCropper from "@/components/video-detail/SmartCropper";
...
export default async function VideoWorkstationPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen text-white">
      {/* Header & Breadcrumbs */}
      <div className="mb-8">
        <Link href="/dashboard" className="text-zinc-400 hover:text-white flex items-center gap-1 text-sm mb-4 inline-flex transition-colors">
          <ChevronLeft size={16} /> Back to Dashboard
        </Link>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">{video.originalName}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl relative group">
             ...
             {isProcessing && (
              <div className="absolute inset-0 bg-zinc-950/80 z-10 flex flex-col items-center justify-center p-8 text-center">
                 ...
                 <p className="text-zinc-400 text-sm mt-2">Your video is being analyzed. Smart crops and AI insights will appear here shortly.</p>
              </div>
             )}
          ...
          {/* Smart Crops Section */}
          <div className={isProcessing ? 'opacity-50 pointer-events-none filter grayscale transition-all' : ''}>
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-violet-500"/>
              Smart Crops (On-the-Fly)
            </h2>
            <SmartCropper publicId={video.publicId} />
          </div>
        </div>

        {/* RIGHT COLUMN: AI Intelligence Layer (1/3 width) */}
        <div className={`lg:col-span-1 h-[calc(100vh-200px)] sticky top-8 ${isProcessing ? 'opacity-50 pointer-events-none filter grayscale transition-all' : ''}`}>
