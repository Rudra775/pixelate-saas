...
export default function FrameTest() {
  ...
  useEffect(() => {
    ...
    timer.current = setInterval(async () => {
      ...
      if (d.state === 'completed' || d.state === 'failed') {
        ...
        timer.current = null; // Mark as cleared
      }
    }, 2000); // Increased interval to 2s
  ...
  const statusDisplay = useMemo(() => {
    ...
    switch (status.state) {
      case 'queued':
        return <p className="text-yellow-500 font-medium">⏳ Job Queued. Waiting for processing...</p>;
      case 'processing':
        return <p className="text-blue-500 font-medium animate-pulse">⚙️ Processing video. Please wait...</p>;
      case 'completed':
        return <p className="text-green-500 font-medium">✅ Processing complete! Best frame shown below.</p>;
      case 'failed':
        return <p className="text-red-500 font-medium">❌ Processing failed. {status.error || 'Please try again.'}</p>;
      default:
        return null;
    }
  }, [status]);
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto bg-gray-900 text-white min-h-screen">
      ...
      <form onSubmit={onSubmit} className="flex items-center space-x-4 p-4 border border-gray-700 rounded-lg bg-gray-800">
      ...
      {jobId && <p className="mt-4 text-sm text-gray-400">Tracking Job ID: {jobId}</p>}
      ...
      {status?.state === 'completed' && status?.result?.url && (
        <div className="mt-6 p-4 border border-green-700 rounded-lg bg-gray-800">
          <p className="font-bold text-lg mb-3 text-green-400">🎉 Best Frame Extracted</p>
          <img 
